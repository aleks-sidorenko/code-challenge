...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    ...
    def check(x: Int, y: Int, expected: Int) =
      assert(boxBlurKernel(src, x, y, 1) == expected,
        s"boxBlurKernel(src, $x, $y) should be $expected " +
          s"but it's ${boxBlurKernel(src, x, y, 1)}")
  ...
  test("HorizontalBoxBlur.parBlur with radius 1 and parallelism level 2 should correctly blur the entire 3x3 image") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
  ...
  test("VerticalBoxBlur.parBlur with radius 2 and parallelism level 4 should correctly blur the entire " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
  ...
  test("VerticalBoxBlur.parBlur with radius 2 and parallelism level 16 should correctly blur the entire " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
